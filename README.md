...
- **Add live solar generation to the startup email too.** The startup email (sent whenever `main.py` starts, e.g. after a restart) uses a separate code path (`log_current_mode_on_startup` / `notify_startup_email`) from mode-change emails, so it didn't pick up the change above automatically. `log_current_mode_on_startup` now also extracts and returns live solar kW from the same startup energy-flow fetch, rendered the same way in the startup email.

**2026-07-14**
- **Retry the SOC/solar fetch used for mode-change emails, to reduce "Unknown" fields.** Investigation into the still-occasional "Battery SOC Unknown" / "Solar Produced Today Unknown" emails (see 2026-07-13 entry) found a genuine, recurring Sigen-side transient failure on the energy-flow endpoint — 446 occurrences of a `502`/`503` "unexpected mimetype" error since 2026-05-15, including a 36-minute burst on 2026-07-13 and a ~15.5-hour degraded window on 2026-07-07 — unrelated to the CloudFront WAF issue fixed on 2026-07-12. Daytime mode-change emails were largely insulated from this because every daytime period handler (`morning.py`/`afternoon.py`/`evening.py`) already fetches SOC earlier in the same tick for its own headroom/export decisions and passes it into `apply_mode_change`, so a failure in the fallback fetch alone didn't blank the email. The night-mode-base path (fixed 2026-07-13) and `solar_generated_today_kwh` (never pre-fetched by any handler, day or night) had no such redundancy — a single failed fetch meant a blank field. `logic/inverter_control.py`'s energy-flow fetch inside `apply_mode_change` now retries up to 3 attempts total, 2 seconds apart, matching the existing auth-retry convention in `main.py`. This won't help against sustained multi-minute Sigen-side outages (those will still correctly show "Unknown"), but should catch the common single-tick blips.

**2026-07-13**
- **Fix: night-to-TOU transition emails always showed "Battery SOC Unknown" and "Solar Produced Today Unknown".** The nightly switch into TOU mode at the cheap-rate boundary (23:00 local) goes through `night.py`'s base night-mode-apply path, which only fetched SOC as a side effect of the *pre-cheap-rate export planning* branch — and that branch only runs while there's still time left before cheap-rate starts. By the time the actual TOU-switch tick fires, `hours_until_cheap_rate` has already reached 0, so that branch (and its SOC fetch) is skipped, leaving `soc=None` for every night-to-TOU transition. `logic/night.py` now fetches SOC directly in the base night-mode-apply path when it wasn't already fetched earlier in the tick, and passes it through to both the decision checkpoint log (previously hardcoded to `soc=None`) and the mode-change email. Also elevated a previously silent `debug`-level exception log in `logic/inverter_control.py`'s fallback energy-flow fetch (used to backfill SOC/solar for every mode-change email) to `warning`, and added explicit warnings for the two other silent-failure cases — a non-dict energy-flow response, and a dict response missing the expected `batterySoc`/solar fields — so future SOC/Solar "Unknown" emails leave a trace in `monitor.log` instead of failing silently.
...
**2026-06-01**
- **Fix: Solcast "outage" alert triggered late-evening when data was available in earlier cache snapshots.** After ~18:30 UTC Solcast stops including today's completed half-hour slots in its API response, so the latest archive snapshot had no today daytime entries and the system fell back to ESB with an alert. Earlier snapshots captured during the day do have the data. `SolcastForecast` now scans backward through the archive and supplements the latest snapshot with today's Morn/Aftn/Eve entries from the most recent earlier snapshot that has them.
- **Fix: high-SOC mid-period export in Amber Evening terminated immediately.** The export triggered at SOC 60.8% (Eve = Amber, headroom deficit 4.99 kWh) but stopped after one tick with "SOC floor reached (59.8% ≤ 75.0%)". Root cause: `maybe_restore_timed_grid_export` was overriding the stored 40% floor with the dynamic period-status-based floor (75% for Amber) even for clipping/high-SOC exports. The dynamic floor is only intended for headroom exports that cross a period boundary mid-run. Clipping and high-SOC exports now use the floor stored at start time (`DAYTIME_TIMED_EXPORT_MIN_SOC_PERCENT = 40%`) rather than the dynamic override.
